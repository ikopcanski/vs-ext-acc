...
namespace CodeContracts.Contrib.Rewriters
{
    internal class ProxyClassConstructorCreator : CSharpSyntaxRewriter
    {
        ...
        {
            var declarators = SyntaxFactory.SeparatedList<VariableDeclaratorSyntax>().Add(SyntaxFactory.VariableDeclarator("_contract"));
            ...
            var parameters = SyntaxFactory.ParseParameterList(string.Format("({0} contract)", _interfaceName));
            var statement = SyntaxFactory.ParseStatement("_contract = contract;");
